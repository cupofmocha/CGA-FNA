...
# Optional: compute a cellularity/interest proxy instead of the original density.
# This is intended for dataset construction (demo.py/make_dataset) or debugging.
# For training-speed, prefer precomputing via tools/augment_interest_scores.py.
try:
...
def get_density(file):
    """Return the per-patch scalar used as "density" in the pipeline.

    By default, this matches the original behavior.
    If CYTOAL_DENSITY_MODE is set to 'interestA' or 'interestB', this will
    compute an interest score from the patch RGB and return that instead.

    Notes:
      - This function is mainly used when building the .npy dataset via make_dataset().
      - For minimal runtime overhead, precompute and store the score into the dataset
        (column 2) using tools/augment_interest_scores.py.
    """

    mode = os.environ.get('CYTOAL_DENSITY_MODE', 'rawsum').lower()
    # Original behavior: normalized sum of pixel intensities
    if mode not in ('interesta', 'interestb') or interest_score is None:
        img_pred = Image.open(file)
        tmp = np.sum(np.array(img_pred)) / (224 * 224 * 255)
        return tmp

    # Interest score mode (A/B)
    rgb = np.asarray(Image.open(file).convert('RGB'), dtype=np.uint8)
    cfg = InterestConfig(method=('A' if mode == 'interesta' else 'B'), gamma=float(os.environ.get('CYTOAL_GAMMA', '0.8')))
    return float(interest_score(rgb, cfg))
